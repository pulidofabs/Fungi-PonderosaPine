...
SppAcum <- specaccum(OtuRare3, method="exact")
...
tiff('Analysis/QualityControl/Graphs/SpeciesAccumulation.pdf', units="in", width=6, height=8, res=800, compression = "lzw")
plot(SppAcum ,xlab="number of samples",ylab="number of ASV's", main="OTURare3")
dev.off()
...
dir.create("Analysis/SpeciesRichness/Fungi")
write.csv(OtuSppRichness, "Analysis/SpeciesRichness/Fungi/EcMCoreMetricsAll.csv") #all above species calculations


#sUBSET ONLY THE RICHNESS METRICS, they are at end of file and one in 1st column.................
ncol(OtuSppRichness)#8592

#1 column and 6th column
CoreMetrics1<-OtuSppRichness[,8587:8592];head(CoreMetrics1)#8593-6 
...
write.csv(MetaRareSpp, "Analysis/Metadata/Fungi/MetaRareCoreEcM.csv")

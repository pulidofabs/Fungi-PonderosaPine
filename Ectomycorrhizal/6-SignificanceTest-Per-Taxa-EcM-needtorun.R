...
#EXPORT FILES .............................................................................
write.csv(RelGenTrt1, "Analysis/RelativeAbundance/Ectomycorrhizal/Tables/RelativeAbundace-Trt-Signif.csv")
write.csv(RelGenTsf1B, "Analysis/RelativeAbundance/Ectomycorrhizal/Tables/RelativeAbundace-TSF-Burn-Signif.csv")
write.csv(RelGenTsf1Un, "Analysis/RelativeAbundance/Ectomycorrhizal/Tables/RelativeAbundace-TSF-Unburn-Signif.csv")
...
CenococcumGLM<-glm(CenococcumB$Abundance ~ CenococcumB$TimeSinceFire + (1/Site),  family= quasipoisson(link = "log"), data = CenococcumB);summary(CenococcumGLM)
CenoAOV<-aov(CenococcumGLM);summary(CenoAOV)#0.0334 *
CenoTK<-TukeyHSD(CenoAOV); CenoTK #None Sign

ChromelosporiumGLM<-glm(ChromelosporiumB$Abundance ~ ChromelosporiumB$TimeSinceFire + (1/Site), data = ChromelosporiumB)
ChromAOV<-aov(ChromelosporiumGLM);summary(ChromAOV) #6.17e-07 ***
ChromTK<-TukeyHSD(ChromAOV); ChromTK # 5-2, 5-3, 11-5

CortinariusGLM<-glm(CortinariusB$Abundance ~ CortinariusB$TimeSinceFire + (1/Site), data = CortinariusB)
CortAOV<-aov(CortinariusGLM);summary(CortAOV) #0.17
CortTK<-TukeyHSD(CortAOV); CortTK

GeopyxisGLM<-glm(GeopyxisB$Abundance ~ GeopyxisB$TimeSinceFire + (1/Site), data = GeopyxisB)
...
HebelomaGLM<-glm(HebelomaB$Abundance ~ HebelomaB$TimeSinceFire + (1/Site), data = HebelomaB)
HebelomaAOV<-aov(HebelomaGLM);summary(HebelomaAOV) #0.14
HebelomaTK<-TukeyHSD(HebelomaAOV); HebelomaTK 

InocybeGLM<-glm(InocybeB$Abundance ~ InocybeB$TimeSinceFire + (1/Site), data = InocybeB)
InocybeAOV<-aov(InocybeGLM);summary(InocybeAOV) #0.0192
InocybeTK<-TukeyHSD(InocybeAOV); InocybeTK#3-2

LyophyllumGLM<-glm(LyophyllumB$Abundance ~ LyophyllumB$TimeSinceFire + (1/Site), data = LyophyllumB)
LyophyllumAOV<-aov(LyophyllumGLM);summary(LyophyllumAOV) #0.0118
LyophyllumTK<-TukeyHSD(LyophyllumAOV); LyophyllumTK #11-3; 11-5

MorchellaGLM<-glm(MorchellaB$Abundance ~ MorchellaB$TimeSinceFire + (1/Site), data = MorchellaB)
MorchellaAOV<-aov(MorchellaGLM);summary(MorchellaAOV) #0.253
MorchellaTK<-TukeyHSD(MorchellaAOV); MorchellaTK #

PezizaGLM<-glm(PezizaB$Abundance ~ PezizaB$TimeSinceFire + (1/Site), data = PezizaB)
PezizaAOV<-aov(PezizaGLM);summary(PezizaAOV)#0.301
PezizaTK<-TukeyHSD(PezizaAOV); PezizaTK

PustulariaGLM<-glm(PustulariaB$Abundance ~ PustulariaB$TimeSinceFire + (1/Site), data = PustulariaB)
...
RhizopogonGLM<-glm(RhizopogonB$Abundance ~ RhizopogonB$TimeSinceFire + (1/Site), data = RhizopogonB)
RhizopogonAOV<-aov(RhizopogonGLM);summary(RhizopogonAOV) #0.111
RhizopogonTK<-TukeyHSD(RhizopogonAOV); RhizopogonTK

SistotremaGLM<-glm(SistotremaB$Abundance ~ SistotremaB$TimeSinceFire + (1/Site), data = SistotremaB)
SistotremaAOV<-aov(SistotremaGLM);summary(SistotremaAOV) #0.524
SistotremaTK<-TukeyHSD(SistotremaAOV); SistotremaTK

ThelephoraGLM<-glm(ThelephoraB$Abundance ~ ThelephoraB$TimeSinceFire + (1/Site), data = ThelephoraB)
ThelephoraAOV<-aov(ThelephoraGLM);summary(ThelephoraAOV) #0.373
ThelephoraTK<-TukeyHSD(ThelephoraAOV); ThelephoraTK

TrichophaeaGLM<-glm(TrichophaeaB$Abundance ~ TrichophaeaB$TimeSinceFire + (1/Site), data = TrichophaeaB)
TrichophaeaAOV<-aov(TrichophaeaGLM);summary(TrichophaeaAOV) #0.147
TrichophaeaTK<-TukeyHSD(TrichophaeaAOV); TrichophaeaTK 

WilcoxinaGLM<-glm(WilcoxinaB$Abundance ~ WilcoxinaB$TimeSinceFire + (1/Site), data = WilcoxinaB)
...
#***********************************************************************************************************-----
AmphinemaGLM<-glm(AmphinemaUn$Abundance ~ AmphinemaUn$TimeSinceFire + (1/Site), data = AmphinemaUn)
AmphinemaAOV<-aov(AmphinemaGLM);summary(AmphinemaAOV) #0.116
AmphinemaTK<-TukeyHSD(AmphinemaAOV); AmphinemaTK#

CortinariusGLM<-glm(CortinariusUn$Abundance ~ CortinariusUn$TimeSinceFire + (1/Site), data = CortinariusUn)
CortAOV<-aov(CortinariusGLM);summary(CortAOV) #0.0159 *
CortTK<-TukeyHSD(CortAOV); CortTK #5-2; 5-3; 

EntolomaGLM<-glm(EntolomaUn$Abundance ~ EntolomaUn$TimeSinceFire + (1/Site), data = EntolomaUn)
EntolomaAOV<-aov(EntolomaGLM);summary(EntolomaAOV) #0.391
EntolomaTK<-TukeyHSD(EntolomaAOV); EntolomaTK #

HelvellosebacinaGLM<-glm(HelvellosebacinaUn$Abundance ~ HelvellosebacinaUn$TimeSinceFire + (1/Site), data = HelvellosebacinaUn)
...
HygrophorusGLM<-glm(HygrophorusUn$Abundance ~ HygrophorusUn$TimeSinceFire + (1/Site), data = HygrophorusUn)
...
InocybeGLM<-glm(InocybeUn$Abundance ~ InocybeUn$TimeSinceFire + (1/Site), data = InocybeUn)
InocybeAOV<-aov(InocybeGLM);summary(InocybeAOV) #0.00175 **
InocybeTK<-TukeyHSD(InocybeAOV); InocybeTK#Only 5-3

PilodermaGLM<-glm(PilodermaUn$Abundance ~ PilodermaUn$TimeSinceFire + (1/Site), data = PilodermaUn)
...
RhizopogonGLM<-glm(RhizopogonUn$Abundance ~ RhizopogonUn$TimeSinceFire + (1/Site), data = RhizopogonUn)
RhizopogonAOV<-aov(RhizopogonGLM);summary(RhizopogonAOV) #0.132
RhizopogonTK<-TukeyHSD(RhizopogonAOV); RhizopogonTK

RussulaGLM<-glm(RussulaUn$Abundance ~ RussulaUn$TimeSinceFire + (1/Site), data = RussulaUn)
RussulaAOV<-aov(RussulaGLM);summary(RussulaAOV) #1.05e-06 ***
RussulaTK<-TukeyHSD(RussulaAOV); RussulaTK # All in comparison to 5

SistotremaGLM<-glm(SistotremaUn$Abundance ~ SistotremaUn$TimeSinceFire + (1/Site), data = SistotremaUn)
...
SuillusGLM<-glm(SuillusUn$Abundance ~ SuillusUn$TimeSinceFire + (1/Site), data = SuillusUn)
SuillusAOV<-aov(SuillusGLM);summary(SuillusAOV)#0.285
SuillusTK<-TukeyHSD(SuillusAOV); SuillusTK #

ThelephoraGLM<-glm(ThelephoraUn$Abundance ~ ThelephoraUn$TimeSinceFire + (1/Site), data = ThelephoraUn)
ThelephoraAOV<-aov(ThelephoraGLM);summary(ThelephoraAOV)#0.0163 *
ThelephoraTK<-TukeyHSD(ThelephoraAOV); ThelephoraTK #only 5-2

TomentellaGLM<-glm(TomentellaUn$Abundance ~ TomentellaUn$TimeSinceFire + (1/Site), data = TomentellaUn)
TomentellaAOV<-aov(TomentellaGLM);summary(TomentellaAOV) #0.0447 *
TomentellaTK<-TukeyHSD(TomentellaAOV); TomentellaTK#5-3

TricholomaGLM<-glm(TricholomaUn$Abundance ~ TricholomaUn$TimeSinceFire + (1/Site), data = TricholomaUn)
...
TuberGLM<-glm(TuberUn$Abundance ~ TuberUn$TimeSinceFire + (1/Site), data = TuberUn)
TuberAOV<-aov(TuberGLM);summary(TuberAOV) #0.000407 ***
TuberTK<-TukeyHSD(TuberAOV); TuberTK #All in comparison to 5

TylosporaGLM<-glm(TylosporaUn$Abundance ~ TylosporaUn$TimeSinceFire + (1/Site), data = TylosporaUn)
...
WilcoxinaGLM<-glm(WilcoxinaUn$Abundance ~ WilcoxinaUn$TimeSinceFire + (1/Site), data = WilcoxinaUn)

...
RawOtu<-read.csv("QiimeOutputs/FunGuild/Fungal-table-with-taxonomy.guilds_editted.csv", row.names = 1) #Raw OTU table w/o singletons, contaminated sample removed
dim(RawOtu)#8585  223
head(RawOtu[1:3,])
...
#SppAcumSaprobe <- specaccum(SaprobeRare3, method="exact")
#plot(SppAcumSaprobe)
...
#capture.output(SppAcumSaprobe, file="Analysis/QualityControl/Saprobes/SaprobeSppAcumData.csv")
...
#pdf("Analysis/QualityControl/Saprobes/SpeciesAccumulation.pdf", width=6, height=8)
#plot(SppAcumSaprobe ,xlab="number of samples",ylab="number of ASV's", main="SaprobeRare3")
#dev.off()
...
#****************************************************************************************************************----
dir.create("Analysis/SpeciesRichness/Saprobes")
...
ncol(SaprobeSppRichness)# 2731

#1 column and 6th column
CoreMetrics1<-SaprobeSppRichness[,2726:2731];head(CoreMetrics1)#2731-5
...
write.csv(MetaRareSpp, "Analysis/Metadata/Saprobes/MetaRareCoreSaprobe.csv")
